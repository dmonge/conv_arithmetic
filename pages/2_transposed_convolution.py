"""Streamlit page for exploring transposed convolution arithmetic."""
...
def _compute_output_size(input_size: int, padding: int, kernel_size: int, stride: int):
    return (input_size - 1) * stride + kernel_size + padding
...
def main():
    st.title('Transposed convolution arithmetic')
    ...
    st.markdown('then, the size of the transposed convolution output, _o_, is:')
    st.latex(r'o = (i - 1) * s + k + p')
    ...
    # plots
    st.subheader('Transposed convolution')
